...
# TODO:            if line far: set gaps from bounds
    ...
    def _updateCurve(self):
        if len(self.tops) == 0 or len(self.bottoms) == 0:
            ...
            y_bot1 = f_bot(xs)
            y_top1 = f_top(xs)
            height = np.mean(y_bot1 - y_top1)
            y_bot1 = y_bot1.astype(int)
            y_top1 = y_bot1 - int(height)
    ...
    def extract(self, img, expands=(0.0,0.0)):
        ...
        y_top_ep = np.clip(y_top_ep,0,img.shape[0]-1)
        y_bot_ep = np.clip(y_bot_ep,0,img.shape[0]-1)
        n = len(xs_ep)
        if len(img.shape) > 2:
            retline = np.zeros((height,n,3), dtype=img.dtype)
        else:
            retline = np.zeros((height,n), dtype=img.dtype)
        for i in range(n): #TODO: remove EmptyGaps
            aa = y_top_ep[i]
            bb = y_bot_ep[i]
            retline[:(bb-aa),i] = img[aa:bb,xs_ep[i]]
        return retline 
...
def extractLines2(imgpath, clf, return_debug=False):
    tt = time()
    ...
    for line in allines:
        line._updateCurve()
        line.expandPoints()
        imgline = line.extract(img_grey, expands=SubLine.EP_FINAL_EXPAND)
        retlines.append(imgline)
        bounds.append({'topleft':line.topleft,
                       'topright':line.topright,
                       'botleft':line.botleft,
                       'botright':line.botright})
    
    return retlines, bounds, illu, img2
